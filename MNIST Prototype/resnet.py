...
def resnet18_encoder(first_conv, maxpool1, weights=None):
    model = ResNetEncoder(EncoderBlock, [2, 2, 2, 2], first_conv, maxpool1)
    if weights:
        model.load_state_dict(weights)
    return model

def resnet18_decoder(latent_dim, input_height, first_conv, maxpool1, weights=None):
    model = ResNetDecoder(DecoderBlock, [2, 2, 2, 2], latent_dim, input_height, first_conv, maxpool1)
    if weights:
        model.load_state_dict(weights)
    return model

def resnet50_encoder(first_conv, maxpool1, weights=None):
    model = ResNetEncoder(EncoderBottleneck, [3, 4, 6, 3], first_conv, maxpool1)
    if weights:
        model.load_state_dict(weights)
    return model

def resnet50_decoder(latent_dim, input_height, first_conv, maxpool1, weights=None):
    model = ResNetDecoder(DecoderBottleneck, [3, 4, 6, 3], latent_dim, input_height, first_conv, maxpool1)
    if weights:
        model.load_state_dict(weights)
    return model